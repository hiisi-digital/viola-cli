...
Deno.test("Config loading - detects viola.config.ts", async () => {
  ...
  assertEquals(
    configExists,
    true,
    "viola.config.ts should exist in valid-config fixture",
  );
});
...
Deno.test("Config loading - detects deno.json viola section", async () => {
  ...
  assertEquals(
    configExists,
    true,
    "deno.json should exist in json-config fixture",
  );
...
Deno.test("Config loading - JSON config has proper structure", async () => {
  ...
  assertEquals(
    Array.isArray(config.viola.include),
    true,
    "Should have include array",
  );
  assertEquals(
    config.viola.include.includes("src"),
    true,
...
Deno.test("Config loading - fixture structure is correct", async () => {
  ...
  for (const fixture of fixtures) {
    ...
    assertEquals(
      exists,
      true,
      `Fixture ${fixture} should exist as a directory`,
    );
  }
});
