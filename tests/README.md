...
- ✅ Boolean flags: `--help`, `--verbose`, `--report-only`, `--list`, `--parallel`
- ✅ String options: `--config`, `--project`, `--only`, `--skip`, `--include`, `--plugins`
...
**Note**: Integration tests gracefully skip when the `@hiisi/viola` module is not available (development environment). They will run fully in production environments where dependencies are properly configured.
...
The `fixtures/` directory contains minimal test projects used by integration tests:
...
Integration tests run the full CLI as a subprocess. In environments where the `@hiisi/viola` module is not available at `../viola/mod.ts`, tests will detect this and skip gracefully with a warning message.

To run integration tests fully:
1. Ensure `@hiisi/viola` is available (either via JSR or local development setup)
